...
key_points:
  - >-
    - Google Cloud C4 VM（Intel Xeon 6プロセッサ搭載）が、前世代のC3 VM（4th Gen Intel
    Xeonプロセッサ搭載）と比較して、OpenAI GPT OSS大規模言語モデル（LLM）のテキスト生成におい
  - '- ## 重要ポイント'
  - >-
    - *   **TCOの劇的な改善:** Google Cloud C4 VMは、GPT OSS LLMの推論において、前世代のC3
    VMと比較してTCOを最大1.7倍改善。
