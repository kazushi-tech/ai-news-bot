// Node >=18 / ESM
// 使い方:
//   node scripts/summarize_url.mjs https://example.com/article
//   node scripts/summarize_url.mjs https://example.com/article --date=2025-11-07 --outdir=articles
...
async function summarizeWithGemini(text, url, modelName = "gemini-2.5-flash") {
  ...
  const prompt = [
    ...
    "Output strictly in the following markdown skeleton:",
    "",
    ...
    "",
    "## Summary",
    "<4-8 short bullets or 2-3 short paragraphs>",
    "",
    `Source: ${url}`,
  ].join("\n");
...
async function main() {
  ...
  if (!args[0] || args[0].startsWith("-")) {
    console.error("Usage: node scripts/summarize_url.mjs <url> [--date=YYYY-MM-DD] [--outdir=articles]");
    process.exit(2);
  }
  const url = args[0];
  ...
  if (!ENV.OFFLINE && ENV.API_KEY) {
    try {
      ...
      console.warn(`[warn] Gemini failed (${err.message}). Falling back to offline summary.`);
...
function offlineSummary(text, url) {
  ...
  const bullets = chunkWords(s, 40).slice(0, 5).map(b => `- ${b}`);
  return [
    `TLDR: ${tldr}`,
    "",
    "## Key Points",
    ...
    "",
    "## Summary",
    chunkWords(s, 80).slice(0, 6).map(x => `- ${x}`).join("\n"),
    "",
    `Source: ${url}`,
  ].join("\n");
}
