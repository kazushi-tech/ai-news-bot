...
const allowHosts = String(args.allow || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
function isJa(s = "") {
  ...
  return ja >= 10 && ja >= en * 0.5;
}
// URLを正規化（トラッキング系クエリ排除）
function canonical(u = "") {
  try {
    const url = new URL(u);
    url.hash = "";
    const kill = /^(utm_|gclid|fbclid|spm|igshid)/i;
    [...url.searchParams.keys()].forEach(
      (k) => kill.test(k) && url.searchParams.delete(k)
    );
    return url.toString();
  } catch {
    return (u || "").trim();
...
if (args.from && args.to) {
  from = parseYmd(args.from);
  to = parseYmd(args.to);
} else {
  to = new Date();
  from = new Date(to);
  from.setDate(to.getDate() - 6); // 直近7日
}
const fromStr = fmt(from);
const toStr = fmt(to);
let rows = all.filter((r) => r.date >= fromStr && r.date <= toStr);
