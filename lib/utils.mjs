...
export async function translateJa(text) {
  const s = (text || '').trim();
  if (!s) return '';
  if (/[一-龠ぁ-んァ-ヶ]/.test(s)) return s; // 日本語ならそのまま
  const p = spawn('python', ['scripts/translate_argos.py'], { stdio: ['pipe','pipe','inherit'] });
  p.stdin.end(s, 'utf8');
  const chunks = [];
  for await (const c of p.stdout) chunks.push(c);
...
// 超シンプル全文取得（APIゼロ）: <article> 優先、なければ <body>
// Node v18+ なら fetch はグローバルにあります（v24 ならOK）
export async function fetchFulltext(url) {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'ai-news-bot (github actions)' }
  });
...
export function htmlToMd(html) {
  ...
  md = md.replace(/<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi,
                  (_, href, text) => `[${strip(text)}](${href})`);
  md = md.replace(/<img[^>]*alt="([^"]*)"[^>]*src="([^"]+)"[^>]*>/gi,
                  (_, alt, src) => `![${alt || ''}](${src})`);
  md = md.replace(/<[^>]+>/g, '');     // 残りタグを掃除
  md = md.replace(/\n{3,}/g, '\n\n');  // 改行詰め
  return md.trim();
}
